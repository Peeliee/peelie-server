...
public class PromptGenerator {
    ...
    public String generatePrompt(PromptCommand command) {

        StringBuilder prompt = new StringBuilder();

        prompt.append("\n\n=== 사용자 프로필 ===\n\n");
        prompt.append("User ID: ").append(command.getUserId()).append("\n");
        prompt.append("사용자 이름: ").append(command.getUserName()).append("\n\n");
        prompt.append("다음은 사용자가 온보딩 과정에서 선택한 관심사와 답변입니다.\n\n");
        prompt.append("사용자를 언급할 때는 '").append(command.getUserName()).append("' 이름을 사용해주세요.\n\n");

        // 각 카테고리별 정보 추가
        for (int i = 0; i < command.getAnswers().size(); i++) {
            PromptCommand.SubCategoryAnswer subCategoryAnswer = command.getAnswers().get(i);
            ...
            // 각 질문과 답변 추가 (L1~L4)
            for (PromptCommand.QuestionAnswer qa : subCategoryAnswer.getQuestionAnswers()) {
